...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log(
      "  --input <path>     Ruta al archivo JSON de datos (por defecto: data/one-pace-data-{lang}.json)"
    );
    console.log(
      "  --output <path>    Directorio de salida (por defecto: ./stremio-addon)"
    );
    console.log("  --no-posters       Desactivar posters/imágenes en el addon");
    console.log("\nEjemplos:");
    console.log("  bun generate-stremio.ts");
    console.log("  bun generate-stremio.ts --lang en --output ./my-addon");
    console.log(
      "  bun generate-stremio.ts --input ./custom-data.json --output ./custom-addon"
    );
    console.log("  bun generate-stremio.ts --no-posters");
    process.exit(0);
  }
  ...
  for (let i = 0; i < args.length; i += 2) {
    ...
    switch (flag) {
      ...
      case "--no-posters":
        usePosters = false;
        i -= 1; // No hay valor para esta opción, decrementar el índice
        break;
      case "--help":
      case "-h":
        console.log("Uso: bun generate-stremio.ts [opciones]");
        console.log("\nOpciones:");
        console.log(
          "  --lang <es|en>     Idioma de los datos (por defecto: es)"
        );
        console.log(
          "  --input <path>     Ruta al archivo JSON de datos (por defecto: data/one-pace-data-{lang}.json)"
        );
        ...
        console.log(
          "  --no-posters       Desactivar posters/imágenes en el addon"
        );
        process.exit(0);
      default:
  ...
  console.log(`🖼️  Posters: ${usePosters ? "Habilitados" : "Deshabilitados"}`);
  console.log("");
  ...
  try {
    ...
    // Generar addon
    const generator = new StremioAddonGenerator(outputPath, usePosters);
    ...
    console.log(
      "  1. Sube la carpeta a un hosting estático (GitHub Pages, Vercel, Netlify)"
    );
